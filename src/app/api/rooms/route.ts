...
export async function GET(request: NextRequest) {
  ...
  try { return json({ rooms: listRooms().map((room) => ({ code: room.code, title: room.title, createdAt: room.createdAt, approved: room.teams.filter((team) => team.status === 'approved').length, active: room.matches.some((match) => match.status === 'active') })) }); }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const room = createRoom(body.title ?? '', body.practiceReuse === true);
